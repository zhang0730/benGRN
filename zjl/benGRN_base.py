...
# Define a function to calculate precision and recall 简单函数计算 precision（精确率）、recall（召回率）
...
#基于 scPRINT 论文的基准测试流程，包括 TF 富集分析、靶基因验证及与 OmniPath 数据库的对比。
...
#训练 Ridge 分类器，用于预测 GRN 与真实网络的映射关系，评估网络质量。
def train_classifier(
    ...
    **kwargs,
    ...
    """
    train_classifier trains a classifier to generate a GRN that maps to the ground truth.   train_classifier 训练一个分类器，以生成一个映射到地面实况的 GRN。

    Uses a RidgeClassifier to select the best combination of networks to predict the ground truth.  使用 RidgeClassifier 来选择预测地面实况的最佳网络组合。
    It is used for the head classification part in the scPRINT paper.  它用于 scPRINT 论文中的头部分类部分。

    Args:
        grn (GRNAnnData): The Gene Regulatory Network data.  基因调控网络数据。
        gt (str, optional): The name of the ground truth database to use. Defaults to "omnipath".  要使用的地面实况数据库名称。默认为 "omnipath"
        other (GRNAnnData, optional): Another GRN to compare against. Defaults to None.   要与之比较的另一个 GRN。默认为 “无”。
        use_col (str, optional): The column name to use for gene symbols. Defaults to "symbol". 用于基因符号的列名。默认为 "symbol"。
        train_size (float, optional): The proportion of the dataset to include in the train split. Defaults to 0.2.  在训练分割中包含的数据集比例。默认为 0.2
        doplot (bool, optional): Whether to plot the results. Defaults to True. 是否绘制结果。默认为 True。
        class_weight (dict, optional): Weights associated with classes in the form {class_label: weight}. Defaults to {1: 200, 0: 1}. 以 {class_label: weight} 的形式与类相关联的权重。默认为 {1: 200, 0: 1}。
        max_iter (int, optional): Maximum number of iterations for the classifier. Defaults to 1_000. 分类器的最大迭代次数。默认为 1_000。
        C (float, optional): Regularization strength; must be a positive float. Defaults to 1.0. 正则化强度；必须是正浮点数。默认为 1.0。
        return_full (bool, optional): Whether to return the full classifier object. Defaults to True. 是否返回完整的分类器对象。默认为 True。
        shuffle (bool, optional): Whether to shuffle the data before splitting. Defaults to False. 是否在分割前对数据进行洗牌： 是否在分割前对数据进行洗牌。默认为 False。

    Returns:
        (GRNAnnData, dict, RidgeClassifier): The Gene Regulatory Network data, the metrics of the classifier, and the classifier object. 基因调控网络数据、分类器指标和分类器对象。
    """
...
#使用 GENIE3 算法从单细胞表达数据（AnnData）推断 GRN，返回 GRNAnnData 对象。
def compute_genie3(
